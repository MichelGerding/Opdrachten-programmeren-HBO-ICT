...
def startStory(story):
    clear_terminal()
    arc = 'intro'

    # while arc:= play_arc(arc, story):
    while arc:
        arc = play_arc(arc, story)
        # de if statements omdat het moet. ik doe het op deze manier 
        # omdat het verhaal wat ik heb gekozen niet goed zich laat
        # bestueren door een vast pad vanwege een recursieve keuze.
        # daarom is gekozen voor het aanroepen die of het nieuwe pad 
        # doorgeeft of de waarde false om het te stoppen
        if True:
        ...
        if False:
            ...
            # geen worden uitgevoerd
            pass



def play_arc(arc, story):
    if arc == 'end':
        return False

    print(f'\n\n{story[arc]["title"]}\n')

    for line in story[arc]['story']:
        print(f'{line}\n')
        sleep(1) 

    print('Paths: ')
    for i, option in enumerate(story[arc]['options']):
        print(f'({i+1}) {option["text"]}')

    while True:
        try:
            user_pick = int(input('pick a path > ').strip())
            if 0  < user_pick < len(story[arc]['options']) +1:  
                return story[arc]['options'][user_pick-1]['arc'] 
        
        except:
            print(f'invalid choice please pick a number between 1 and ' +
                      str(len(story[arc]['options'])+1))
...
story = {
    ...
    "new-york": {
        ...
        "options": [
            ...
            {
                "text": "This is getting too weird for me. Let's bail and head to Denver.",
                "arc": "denver"
            },
            {
    ...
    "debate": {
        ...
        "options": [
            ...
            {
                "text": "Slip out the back before anyone asks us to pick a side and go to Denver",
                "arc": "denver"
            }
        ]
    },
    ...
    "denver": {
        ...
        "options": [
            ...
            {
                "text": "Lets do some more exploring and also Visit New York",
                "arc": "new-york"
            }
        ]
    },
